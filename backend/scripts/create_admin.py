#!/usr/bin/env python3
"""
Script de création du compte administrateur pour CAMEG-CHAIN
Version qui utilise le script standalone pour éviter les problèmes d'encodage
"""
import os
import sys
import logging
...
def create_admin(email: str, password: str, full_name: str = "Administrateur CAMEG"):
    """Créer le compte administrateur"""
    
    logger.info("🔐 Début de la création de l'administrateur")
    ...
    try:
        ...
        try:
            ...
            if existing_user:
                ...
                logger.info(f"✅ Utilisateur {email} mis à jour en tant qu'ADMIN")
                return {
                    "success": True, 
                    "message": f"Utilisateur {email} mis à jour en tant qu'ADMIN",
            ...
            logger.info(f"✅ Administrateur {email} créé avec succès")
            return {
                "success": True, 
                "message": f"Administrateur {email} créé avec succès",
                "user_id": user_id
            }
...
def verify_admin(email: str):
    """Vérifier que l'administrateur a été créé correctement"""
    try:
        engine = create_database_connection()
        SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)
        db = SessionLocal()
...
def main():
    """Fonction principale"""
    print("=" * 70)
    print("🔐 CRÉATION DU COMPTE ADMINISTRATEUR CAMEG-CHAIN")
    print("=" * 70)
    
    # Informations fournies
    email = "obed11@gmail.com"
    password = "Very@Hard//4Me.88"
    full_name = "Administrateur CAMEG"
    ...
    result = create_admin(email, password, full_name)
    ...
    if result["success"]:
        ...
        if verify_admin(email):
            ...
            print("   • Gestion des évaluateurs")
            print("   • Monitoring et rapports")
